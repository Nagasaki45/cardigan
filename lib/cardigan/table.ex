defmodule Cardigan.Table do
  ...
  """
  def start_link({id, %Cardigan.Game{} = game}, opts \\ []) when is_binary(id) do
  ...
  """
  def subscribe(id) do
    Phoenix.PubSub.subscribe(Cardigan.PubSub, id)
  end
  ...
  # Modifiers mimic `Cardigan.Game` functions.
  ...
  def handle_call({:modifier, func, args}, _from, {id, game}) do
    try do
      apply(Cardigan.Game, func, [game | args])
    rescue
      e ->
    ...
    |> case do
      {:ok, game} ->
        Phoenix.PubSub.broadcast(Cardigan.PubSub, id, :table_updated)
